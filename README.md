# Va2PaAstra — Sandboxie-Plus 赞助者证书一键工具（Win32 GUI）

为 Sandboxie-Plus 生成节点锁定（本机 HWID）的赞助者证书，并通过 Astra64 驱动
...
└── third_party/astra/      # 从 Astra64-RW/cpp 复制的 header-only 依赖
    ├── astra.h             # ASTRA64.sys 驱动封装（物理 R/W）
    ├── kernel.h            # 页表遍历 / CR3 查找 / 虚拟地址 R/W
    └── pe.h                # PE 映像辅助
```

设计要点：核心逻辑（`core.cpp`）与表现层（`main.cpp`）分离，核心通过
`va2pa::SetLogger()` 注册的回调输出 UTF-8 日志，UI 将其转为 UTF-16 显示，
因此不存在 ANSI/GBK 乱码问题。

## 依赖

- Visual Studio 2022（MSVC x64，含 Windows SDK），CMake ≥ 3.15
- `ASTRA64.sys`（来自 `D:\Sandboxie\Astra64-RW\ASTRA64.sys`）——
  构建时若源码树中存在该文件会自动复制到输出目录；工具运行时从自身目录加载。
...
产物：`build\Release\Va2PaAstra.exe`（+ 自动复制的 `ASTRA64.sys`）。
